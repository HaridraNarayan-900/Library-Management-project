...
def db_create_book(data):
    ...
    with get_connection() as conn:
        cur = conn.execute("""
            INSERT INTO books (title, author, isbn, category, total_copies, available_copies, published_year, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            data["title"],
            data["author"],
...
def db_update_book(book_id, data):
    ...
    with get_connection() as conn:
        conn.execute("""
            UPDATE books
            SET title = ?, author = ?, isbn = ?, category = ?, total_copies = ?, available_copies = ?, published_year = ?, updated_at = ?
            WHERE id = ?
        """, (
